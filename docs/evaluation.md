...
- facts are sourced from installed adapters or an explicitly labeled evaluation input;
- owner intent becomes a bounded, reviewable workflow plan;
...
Unit tests use injected test doubles and do not replace the live Jev or live Superset proofs. The heterogeneous contract test combines Superset, SQL, Airflow, and table-style source refs without requiring those services to be installed.
...
- recipient correction and workflow revision rate.
...
- rejects ambiguous numeric Superset results instead of guessing a metric;
