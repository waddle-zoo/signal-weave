...
| Change lifecycle | The latest discovery receipt is persisted on the card, but corrections still need a stable snapshot/diff story. | Persist catalog snapshot/version, principal, candidates, judgments, corrections, and card version in a replayable onboarding receipt. |
