...
| Change lifecycle | The latest discovery receipt and caller corrections are persisted on the card, but a stable snapshot/diff view between versions is still open. | Persist catalog snapshot/version, principal, candidates, judgments, corrections, and card version in a replayable onboarding receipt. |
