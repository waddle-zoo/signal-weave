...
| --- | ---: | --- | --- |
| Full regression suite | **198 passed, 1 skipped** | The current contracts, adapters, auth boundary, onboarding packet, gates, receipts, and evaluation harness remain internally consistent. | Unit tests are not production evidence. |
...
| Live Jev onboarding replay | **8/8 required-candidate recall, 8/8 safe outcomes, 7/8 exact recommendation sets, 0 tenant leaks** | Real Jev ranked eight heterogeneous onboarding cases without expected labels in its input. | A bounded sample; the one mismatch was intentionally held as a definition conflict for human review. |
| Current deterministic research rerun | **100/144 exact**, 0 unsafe automatic actions, complete workflow/card/provenance/source-selection contracts | Verifies current MCP wiring, provenance, and fail-closed behavior across 144 sessions. | Not Jev accuracy evidence; 44 mismatches were 22 `insufficient_data -> ignore` and 22 `notify -> investigate`. |
| Historical live Jev large-scale matrix | **144/144 exact**, 0 wrong actions, 288 requests, median 856 ms, p95 1,111 ms | Records a pre-refactor execution baseline. | Historical; synthetic labels and repeated fixture structure. Do not use as current enterprise proof. |
